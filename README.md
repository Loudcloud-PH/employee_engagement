...
1. **Database host is hardcoded.** The `default` block in `config/database.yml` sets `host: localhost`, so the app cannot reach a Coolify-managed PostgreSQL running in another container without first editing `database.yml` to read a host from `ENV` (e.g. `DB_HOST` or `DATABASE_URL`). Resolve this before the first deploy.
2. **`config.force_ssl = true`** redirects plain-HTTP requests to HTTPS. The Coolify internal health check hits the app over HTTP; if it reports 30x/failing, disable `force_ssl` (TLS terminates at the Coolify proxy) or point the health check at the HTTPS listener. A ready-made exclude for `/up` is commented at `config/environments/production.rb:55`.
